...
export const CreateTokens = (someTokens: TokensEthApiModel): { tokens: EthTokenArr, totalSum: Number|string } => {
    let totalSum = 0;
    ...
    if (someTokens.tokens) {
        someTokens.tokens.forEach((item) => {
            if (item.tokenInfo.price) {
                const itemBalance = Number(item.balance) / Math.pow(10, Number(item.tokenInfo.decimals));
